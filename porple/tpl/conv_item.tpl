				...
				<div class="p-2 clearfix wall-item-head{{if $item.is_new && !$item.title && !$item.event && !$item.is_comment}} wall-item-head-new rounded-top{{/if}}">
					<div class="wall-item-info " id="wall-item-info-{{$item.id}}" >
						<div class="wall-item-photo-wrapper{{if $item.owner_url}} wwfrom{{/if}} h-card p-author" id="wall-item-photo-wrapper-{{$item.id}}">
							<img src="{{$item.thumb}}" class="wall-item-photo{{$item.sparkle}} u-photo p-name" id="wall-item-photo-{{$item.id}}" alt="{{$item.name}}"  />
					...
					<div class="wall-item-author">
						...
						{{if $item.thread_author_menu}}
							<i class="fa fa-gear author-gear cursor-pointer" data-toggle="dropdown"></i>
				...
				<div class="p-2 clearfix wall-item-tools">
					<div class="float-right wall-item-tools-right">
						...
						<div class="btn-group">
							...
							{{/if}}
							{{if $item.share}}
								<a class="btn btn-outline-secondary btn-sm" href="#" onclick="jotShare({{$item.id}},{{$item.item_type}}); return false"><i class="generic-icons-nav fa fa-fw fa-retweet" title="{{$item.share.0}}"></i></a>
							{{/if}}
							{{if $item.dislike}}
